...
{
    internal sealed class OpenAiImageDescriptor(string token) : IAiImageDescriptor
    {
        private const string ApiHost = "https://api.openai.com/v1";
        private const string GetImageDescriptionModel = "gpt-4-vision-preview";

        public async Task<string> GetImageDescription(Uri imageUrl, string question, string systemMessage = "")
        {
            var imageDescriptionResponse = await Utils.DoRequest<ChatCompletion>(new
                {
                    ...
                    max_tokens = 512
                }, $"{ApiHost}/chat/completions", token).ConfigureAwait(false);
